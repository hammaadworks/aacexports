...
export function VerticalPageWrapper({
    ...
    return (<div className={cn("relative min-h-screen bg-[#FDFCF8] text-foreground overflow-hidden", className)}>
        {/* Background Texture */}
        <div
...
export function VerticalHero({
    ...
    return (<section className="relative pt-22 pb-16 lg:pt-24 lg:pb-16 px-6 overflow-hidden">
        <div className="container mx-auto text-center relative z-10">
            ...
            >
                ...
                <h1
                    className={cn("text-4xl md:text-7xl font-serif font-bold mb-6 tracking-tighter text-secondary", titleClassName)}
                >
                    {title}
                </h1>
                <p
                    className={cn("text-lg text-muted-foreground max-w-2xl mx-auto leading-relaxed mb-8", descriptionClassName)}
                >
                    {description}
                </p>
 ...
 */
export function VerticalStats({items, className, variant = "icons"}: Readonly<VerticalStatsProps>) {
    return (<section className={cn("py-16 border-y bg-white/50 backdrop-blur-sm", className)}>
        <div className="container mx-auto px-6">
            <div
                className={cn("grid gap-8 text-center", items.length === 3 ? "md:grid-cols-3" : "grid-cols-2 md:grid-cols-4")}>
                {items.map((item, index) => (<div
                    key={item.id}
                    className={cn("px-4 relative", // Add dividers except for the last item (desktop only)
                        index === items.length - 1 ? "" : "md:after:content-[''] md:after:absolute md:after:right-0 md:after:top-1/4 md:after:h-1/2 md:after:w-px md:after:bg-gray-200")}
                >
                    {variant === "icons" && item.icon ? (<>
                        <div
                            className={cn("mx-auto h-14 w-14 rounded-full flex items-center justify-center mb-4 transition-transform hover:scale-110 duration-300", item.colorClass || "bg-primary/10 text-primary")}>
                            <item.icon size={28}/>
                        </div>
                        <h3 className="font-bold text-lg mb-2 text-secondary">{item.label}</h3>
                        {item.description && (
                        ...
                        <p className="text-4xl md:text-5xl font-bold font-serif text-secondary">
                            {item.valuePrefix}
                            {item.value === undefined ? (<span>0</span>) : (<NumberTicker value={item.value}/>)}
                            {item.valueSuffix}
                        </p>
...
export function VerticalSectionHeader({
                                          title, description, badge, className, align = "center"
                                      }: Readonly<{
    title: ReactNode; description?: ReactNode; badge?: string; className?: string; align?: "left" | "center" | "right";
}>) {
    return (<div className={cn("mb-16 max-w-3xl", align === "center" ? "mx-auto text-center" : "", className)}>
        {badge && (<span className="text-[#8B9D77] font-bold tracking-wider uppercase text-sm mb-3 block">
                    {badge}
                </span>)}
...
export function VerticalInfoSection({
    ...
    const textClass = variant === "dark" ? "text-gray-300" : "text-muted-foreground";
    const titleClass = variant === "dark" ? "text-white" : "text-foreground";

    return (<section className={cn("py-20 relative overflow-hidden", variants[variant], className)}>
        <div className="container mx-auto px-6 relative z-10">
            <div className="grid md:grid-cols-2 gap-10 lg:gap-16 items-center">
                <div className="order-2 md:order-1 flex flex-col justify-center items-start h-full">
                    {badgeIcons && badgeText && <PageHeaderBadge icon={badgeIcons}>{badgeText}</PageHeaderBadge>}
                    <h2 className={cn("text-4xl md:text-5xl font-serif font-bold mb-6", titleClass)}>
                        {title}
                    </h2>
                    {description && (<p className={cn("text-lg mb-8 leading-relaxed", textClass)}>
                        {description}
                    </p>)}
                ...
                <div className="order-1 md:order-2">
                    ...
                    <div
                        className={cn("relative rounded-2xl overflow-hidden transition-all duration-500", variant === 'dark' ? "shadow-[0_0_40px_rgba(255,255,255,0.05)] border border-white/10" : "shadow-2xl border border-border/50 bg-white")}>
                        {visual}
                    </div>
                </div>
            </div>
...
export function CatalogViewer({
    ...
    isOpen: boolean; onClose: () => void; doc: CatalogDoc | null;
}>) {
    ...
    return (<BaseModal
        ...
        footerContent={doc && (<div
            ...
            <Button asChild variant="default">
                <a
                    href={`/aacexports/catalogs/${doc.file}`}
                    target="_blank"
    ...
    >
        ...
        >
            {doc && (<Document
                file={`/aacexports/catalogs/${doc.file}`}
            ...
            >
                {numPages && Array.from(new Array(numPages), (_, index) => (<Page
                    key={`page_${index + 1}`}
                    pageNumber={index + 1}
...
export default function VerticalLayout({children}: Readonly<{ children: ReactNode }>) {
    return <>{children}</>;
}
