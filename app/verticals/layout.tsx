...
import { LucideIcon, Maximize2, PackageOpen, Loader2 } from "lucide-react";
...
export function VerticalPageWrapper({
    ...
    return (
        <div className={cn("relative min-h-screen bg-[#FDFCF8] text-foreground overflow-hidden", className)}>
            {/* Background Texture */}
            <div
...
export function VerticalHero({
    ...
    return (
        <section className="relative pt-24 pb-24 lg:pt-42 lg:pb-32 px-8 overflow-hidden">
 ...
 */
export function VerticalStats({ items, className, variant = "icons" }: VerticalStatsProps) {
    return (
        <section className={cn("py-16 border-y bg-white/50 backdrop-blur-sm", className)}>
                        ...
                        >
                            {variant === "icons" && item.icon ? (
                                <>
                                    ...
                                    <h3 className="font-bold text-lg mb-2 text-[#2D3028]">{item.label}</h3>
                                    {item.description && (
                                ...
                                <div className="space-y-2">
                                    <p className="text-4xl md:text-5xl font-bold font-serif text-[#2D3028]">
                                        {item.valuePrefix}
                                        {item.value !== undefined ? (<NumberTicker value={item.value} />) : (
                                            <span>0</span>)}
                                        {item.valueSuffix}
                                    </p>
...
export function VerticalSectionHeader({
    ...
    return (
        <div className={cn("mb-16 max-w-3xl", align === "center" ? "mx-auto text-center" : "", className)}>
            {badge && (
                <span className="text-[#8B9D77] font-bold tracking-wider uppercase text-sm mb-3 block">
                    {badge}
                </span>
            )}
            <h2 className="text-4xl md:text-5xl font-serif font-bold mb-6 text-[#2D3028]">
                {title}
            </h2>
            {description && (
...
export function VerticalInfoSection({
    ...
    return (
        <section className={cn("py-24 relative overflow-hidden", variants[variant], className)}>
            <div className="container mx-auto px-6 relative z-10">
                <div className="grid md:grid-cols-2 gap-12 lg:gap-20 items-center">
                    <div className="order-2 md:order-1">
                        <h2 className={cn("text-3xl md:text-4xl font-serif font-bold mb-6", titleClass)}>
                            {title}
                        </h2>
                        {description && (
                    ...
                    <div className="order-1 md:order-2">
                        ...
                        <div className={cn(
                            ...
                            variant === 'dark' ? "shadow-[0_0_40px_rgba(255,255,255,0.05)] border border-white/10" : "shadow-2xl border border-border/50 bg-white"
                        )}>
                            {visual}
                        </div>
                    </div>
    ...
    return (
            ...
            footerContent={doc && (
                <div className="w-full flex justify-end px-6 py-2 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 border-t">
                    <Button asChild variant="default">
                        <a
                            href={`/aacexports/catalog/${doc.file}`}
                            target="_blank"
        ...
        >
            ...
            >
                {doc && (
                    <Document
                        file={`/aacexports/catalog/${doc.file}`}
