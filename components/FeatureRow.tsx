...
export const FeatureRow = ({
    ...
    return (<div className={cn("grid gap-12 items-center mb-24 last:mb-0", showVisual ? "lg:grid-cols-2" : "grid-cols-1")}>
            <div className={cn("order-2", showVisual && (imageAlign === "right" ? "lg:order-1" : "lg:order-2"))}>
        ...
        {showVisual &&
            <div
                className={cn("order-1 lg:order-2 h-64 lg:h-96 rounded-2xl overflow-hidden relative group", imageAlign === "right" ? "lg:order-2" : "lg:order-1")}>
                {customVisual || (<>
                        ...
                        {image ? (<img src={image} alt={title}
                                ...
                                <div className="absolute inset-0 flex items-center justify-center">
                                     <span
                                         className="text-4xl font-serif font-bold opacity-10 uppercase tracking-widest px-4 text-center">{title}</span>
                                </div>
                            </>)}
                    </>)}

                {/* Decorative Pattern overlay (subtle on images) */}
                {!customVisual && <DotPattern
                    className={cn("absolute inset-0 text-foreground/20", image ? "opacity-30" : "opacity-100")}
