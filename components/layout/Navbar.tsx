...
import { Home, Layers, FileText, Mail, Info } from "lucide-react"; // Added Info icon for About
...
export function Navbar() {
  ...
  return (
    <div className="fixed bottom-6 left-0 right-0 z-50 flex justify-center pointer-events-none">
      <div className="pointer-events-auto flex items-center gap-1 p-1.5 rounded-full border border-border/20 bg-primary shadow-2xl ring-1 ring-black/5 mx-4">
        {navItems.map((item) => {
            ...
            >
              ...
              {hoveredTab === item.name && !isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 rounded-full bg-primary/20" // Darker primary hover
                  transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
                />
              )}
